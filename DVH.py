...
class DHV:
    ...
    Attributes:
        ...
        names (list of str): List of names for each mask/structure.
        masks (list of np.ndarray): List of boolean masks for regions of interest.
        dvhs (list of np.ndarray): List of computed DVHs for each mask.
        dmax (list of float): List of maximum doses for each mask.
        dmin (list of float): List of minimum doses for each mask.  
    Methods:
        computeDVH(mask): Computes the DVH for a given mask.    
        getAllDVH(): Computes DVHs for all masks.
        print_DVHs(): Plots all computed DVHs.
    """


    def __innit__(self, dosemap:np.ndarray, names:list[str], masks:list[np.ndarray], max_DVH:float=100.0) :
